...
class JobsResource(Resource):
    def get(self, job_id):
        ...
        session = db_session.create_session()
        job = session.query(Jobs).get(job_id)
        return jsonify({'job': job.to_dict(only=('id', 'team_leader', 'job', 'work_size', 'collaborators', 'start_date', 'end_date', 'is_finished'))})
...
class JobsListResource(Resource):
    ...
    def post(self):
        args = jobs_parser.parser.parse_args()
        session = db_session.create_session()
        job = Jobs(
            id=int(args['job_id']),
            team_leader=aint(args['team_leader']),
            job=args['job'],
