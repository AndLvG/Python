...
@app.route("/jobs_table", methods=['GET', 'POST'])
def jobs_table():
    session = db_session.create_session()
    if request.method == 'GET':
        ...
        if request.form['action'] == 'text' and request.form['search_str'] != "":
            j = session.query(jobs.Jobs).filter(jobs.Jobs.job.like(f"%{request.form['search_str']}%"))
            ...
            if option == 'all':
                ...
                j = session.query(jobs.Jobs).filter(jobs.Jobs.is_finished == 0)
            elif option == 'other':
                j = session.query(jobs.Jobs).filter(jobs.Jobs.is_finished == 1)

        elif request.form['action'] == 'data':
            j = session.query(jobs.Jobs).filter(
                jobs.Jobs.start_date.between(request.form['datemin'], request.form['datemax'])).order_by(
                jobs.Jobs.start_date.desc())
